...
describe("Task 35: Migration Smoke Test", () => {
  it("all migration files apply sequentially on empty DB without errors", () => {
    ...
    expect(tableNames).toContain("parent_report_settings");
    expect(tableNames).toContain("task_submissions");
    expect(tableNames).toContain("task_sets");
    ...
    expect(tableNames).toContain("child_task_wall_orders");
    ...
    const aiRetention = db.prepare("SELECT value FROM system_settings WHERE key='ai_job_retention_days'").first() as any;
    expect(aiRetention?.value).toBe("92");
    ...
    expect(taskColumnNames).toContain("required_remedy_enabled");
    expect(taskColumnNames).toContain("required_remedy_condition");
    expect(taskColumnNames).toContain("required_remedy_points");
    expect(taskColumnNames).toContain("required_remedy_deadline_hours");
    expect(taskColumnNames).toContain("grading_mode");
    expect(taskColumnNames).toContain("completion_standards_json");
    expect(taskColumnNames).toContain("submission_deadline_json");
