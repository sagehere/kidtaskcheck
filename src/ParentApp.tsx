...
function TaskSetForm({ item, tasks, children, onSave, onCancel }: { item?: TaskSet; tasks: Task[]; children: Child[]; onSave: (data: any) => void; onCancel?: () => void }) {
  const [data, setData] = useState(() => ({ title: item?.title || "", description: item?.description || "", iconValue: item?.icon_value || "🧩", isActive: item?.is_active !== 0, taskIds: item?.taskIds || [] as string[], settlementMode: item?.settlementMode || item?.settlement_mode || "round", windowType: item?.windowType || item?.window_type || "custom", windowStart: item?.windowStart || item?.window_start || "", windowEnd: item?.windowEnd || item?.window_end || "", windowWeekdays: item?.windowWeekdays || normalizeWeekdaysLocal(item?.window_weekdays) }));
  const permittedPeriods = data.settlementMode !== "window" || data.windowType === "custom" ? null : data.windowType === "weekly" ? ["daily", "weekly"] : ["daily", "weekly", "monthly"];
  const selectable = tasks.filter((task) => task.is_active !== 0 && task.point_type === "earn" && (!task.taskSetId || task.taskSetId === item?.id) && (!permittedPeriods || permittedPeriods.includes(task.period)));
  ...
  return (
    <form className="stack" onSubmit={(event) => { event.preventDefault(); onSave({ ...data, iconType: "emoji" }); }}>
      ...
      {data.settlementMode === "window" && <><Field label="周期范围"><select value={data.windowType} onChange={(e) => { const windowType = e.target.value; const allowed = windowType === "weekly" ? ["daily", "weekly"] : windowType === "monthly" ? ["daily", "weekly", "monthly"] : null; setData({ ...data, windowType, taskIds: allowed ? data.taskIds.filter((taskId) => tasks.find((task) => task.id === taskId && allowed.includes(task.period))) : data.taskIds }); }}><option value="custom">指定日期</option><option value="weekly">每周自动重置</option><option value="monthly">每月自动重置</option></select></Field>{data.windowType === "custom" ? <div className="grid two compact-fields"><Field label="开始日期"><input required type="date" value={data.windowStart} onChange={(e) => setData({ ...data, windowStart: e.target.value })} /></Field><Field label="结束日期"><input required type="date" value={data.windowEnd} onChange={(e) => setData({ ...data, windowEnd: e.target.value })} /></Field></div> : <small>{data.windowType === "weekly" ? "按自然周自动结算并重置，仅可加入每日或每周任务。" : "按自然月自动结算并重置，可加入每日、每周或每月任务。"}</small>}<Field label="参与星期"><WeekdayPicker value={data.windowWeekdays} onChange={(windowWeekdays) => setData({ ...data, windowWeekdays })} /></Field><small>{data.windowType === "custom" ? "范围内所选星期按子任务自身周期完成；结束后统一结算。" : "当前周期立即开始；首个已开始的子任务周期会在下次重置后纳入。"}</small></>}
