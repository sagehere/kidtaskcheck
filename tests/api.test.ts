...
describe("Real Child Session Integration", () => {
  ...
  it("stores validated task deadline rules through the parent API", async () => {
    ...
    const dailyPayload = { ...payload, title: "Daily deadline", period: "daily", submissionDeadline: { time: "18:00" } };
    expect((await safe(handleParentRoutes, "/tasks", "POST", makeRequest("POST", "/tasks", dailyPayload), env, parent))!.status).toBe(200);
    const dailyTask = (await (await safe(handleParentRoutes, "/tasks", "GET", makeRequest("GET", "/tasks"), env, parent))!.json()).data.find((item: any) => item.title === "Daily deadline");
    expect(dailyTask.submissionDeadline).toEqual({ time: "18:00" });
    expect((await safe(handleParentRoutes, `/tasks/${dailyTask.id}`, "PATCH", makeRequest("PATCH", `/tasks/${dailyTask.id}`, { ...dailyPayload, submissionDeadline: { time: "17:00" } }), env, parent))!.status).toBe(200);
    const updatedDailyTask = (await (await safe(handleParentRoutes, "/tasks", "GET", makeRequest("GET", "/tasks"), env, parent))!.json()).data.find((item: any) => item.id === dailyTask.id);
    expect(updatedDailyTask.submissionDeadline).toEqual({ time: "17:00" });
    const invalidDaily = await safe(handleParentRoutes, "/tasks", "POST", makeRequest("POST", "/tasks", { ...dailyPayload, title: "Invalid daily deadline", submissionDeadline: { time: "24:00" } }), env, parent);
...
describe("Parent delegate accounts", () => {
  ...
  it("imports config as disabled and ignores missing child assignments", async () => {
    const parentActor = { type: "user", role: "parent", id: pid, displayName: "Parent A", operatorLabel: "妈妈" };
    const payload = {
      tasks: [{ title: "Imported Task", category_name: "Cat", period: "daily", points: 2, submission_deadline: { time: "18:00" }, assignee_names: ["Child A", "Missing"], is_active: 1 }],
    ...
    expect(env.DB.prepare("SELECT submission_deadline_json FROM tasks WHERE title='Imported Task'").first()).toMatchObject({ submission_deadline_json: JSON.stringify({ time: "18:00" }) });
  ...
  it("exports required task settings and child assignment names", async () => {
    const parentActor = { type: "user", role: "parent", id: pid, displayName: "Parent A", operatorLabel: "妈妈" };
    env.DB.prepare("UPDATE tasks SET period='daily', is_required=1, required_count=3, required_penalty_points=4, required_remedy_enabled=1, required_remedy_condition='收拾书桌', required_remedy_points=2, required_remedy_deadline_hours=12, submission_deadline_json=? WHERE id=?").bind(JSON.stringify({ time: "18:00" }), tid).run();
    const rid = id();
    ...
    expect(task).toMatchObject({
      ...
      submission_deadline: { time: "18:00" },
...
describe("Config groups", () => {
  ...
  beforeEach(async () => {
    ...
    env.DB.prepare("INSERT INTO tasks (id, parent_id, category_id, title, description, points, period, limit_count, point_type, enabled_weekdays, is_required, required_count, required_penalty_points, submission_deadline_json) VALUES (?, ?, 'cg-cat', 'Read', 'Read book', 6, 'daily', 2, 'earn', '[1,2,3,4,5]', 1, 2, 3, ?)").bind(tid, pid, JSON.stringify({ time: "18:00" })).run();
    env.DB.prepare("INSERT INTO task_assignees (task_id, child_id) VALUES (?, ?)").bind(tid, cid).run();
  ...
  it("saves and activates a config group as an overwrite snapshot", async () => {
    ...
    expect(tasks[0]).toMatchObject({ is_required: 1, required_count: 2, required_penalty_points: 3, submission_deadline_json: JSON.stringify({ time: "18:00" }) });
...
describe("Child schedule plan text", () => {
  ...
  it("includes schedule plans in weekly reports", async () => {
    ...
    expect(html).toContain("@page{size:A4");
    expect(html).toContain("当前日程模板（参考）");
    expect(html).toContain("Review notes");
    expect(html).toContain("print-task-card");
  });
  it("reports reviewed approval rate, localized details, and previous-period comparison", async () => {
    const approvedId = id();
    const rejectedId = id();
    const pendingId = id();
    env.DB.prepare("INSERT INTO task_submissions (id, task_id, child_id, parent_id, period_key, submitted_at, status, review_note) VALUES (?, ?, ?, ?, '2026-06-02', '2026-06-02 00:00:00', 'approved', '做得很好')").bind(approvedId, tid, cid, pid).run();
    env.DB.prepare("INSERT INTO task_submissions (id, task_id, child_id, parent_id, period_key, submitted_at, status, review_note) VALUES (?, ?, ?, ?, '2026-06-03', '2026-06-03T00:00:00.000Z', 'rejected', '需要更认真')").bind(rejectedId, tid, cid, pid).run();
    env.DB.prepare("INSERT INTO task_submissions (id, task_id, child_id, parent_id, period_key, submitted_at, status) VALUES (?, ?, ?, ?, '2026-06-04', '2026-06-04T00:00:00.000Z', 'pending')").bind(pendingId, tid, cid, pid).run();
    env.DB.prepare("INSERT INTO point_ledger (id, child_id, parent_id, amount, source_type, source_id, period_key, note, created_at) VALUES (?, ?, ?, 5, 'task', ?, '2026-06-02', '任务审核通过', '2026-06-08T00:00:00.000Z')").bind(id(), cid, pid, approvedId).run();
    const req = makeRequest("GET", `/children/${cid}/report?period=weekly&anchor=2026-06-03T00:00:00.000Z`);
    const report = await safe(handleParentRoutes, `/children/${cid}/report`, "GET", req, env, parentActor(), new URL(req.url));
    const html = await report!.text();
    expect(html).toContain("50%");
    expect(html).toContain("待审核");
    expect(html).toContain("需要更认真");
    expect(html).toContain("+5");
    expect(html).toContain("与上一周期对比");
    expect(html).toContain("2026-06-01 至 2026-06-07");
  });
  it("prints only active rules with localized task, reward, and remedy details", async () => {
    env.DB.prepare("UPDATE tasks SET is_required=1, required_count=2, required_penalty_points=3, grading_mode='completion', completion_standards_json='[{\"label\":\"认真完成\",\"points\":5}]' WHERE id=?").bind(tid).run();
    const disabledTaskId = id();
    env.DB.prepare("INSERT INTO tasks (id, parent_id, category_id, title, points, period, limit_count, point_type, enabled_weekdays, is_active) VALUES (?, ?, 'sched-cat', 'Disabled task', 1, 'weekly', 1, 'earn', '[1]', 0)").bind(disabledTaskId, pid).run();
    env.DB.prepare("INSERT INTO task_assignees (task_id, child_id) VALUES (?, ?)").bind(disabledTaskId, cid).run();
    const rewardId = id();
    env.DB.prepare("INSERT INTO rewards (id, parent_id, title, cost_points, limit_period, limit_count, redeem_weekdays, is_active) VALUES (?, ?, 'Movie', 20, 'weekly', 1, '[6]', 1)").bind(rewardId, pid).run();
    env.DB.prepare("INSERT INTO reward_assignees (reward_id, child_id) VALUES (?, ?)").bind(rewardId, cid).run();
    env.DB.prepare("INSERT INTO reward_prerequisites (reward_id, task_id, required_count) VALUES (?, ?, 2)").bind(rewardId, tid).run();
    env.DB.prepare("INSERT INTO feedback_templates (id, parent_id, kind, title, points, is_active, is_remediable, remedy_condition, remedy_points, remedy_deadline_hours) VALUES (?, ?, 'criticism', 'Late', 4, 1, 1, '补做', 2, 12)").bind(id(), pid).run();
    env.DB.prepare("INSERT INTO feedback_templates (id, parent_id, kind, title, points, is_active) VALUES (?, ?, 'praise', 'Disabled praise', 1, 0)").bind(id(), pid).run();
    const print = await safe(handleParentRoutes, `/children/${cid}/export-print`, "GET", makeRequest("GET", `/children/${cid}/export-print`), env, parentActor());
    const html = await print!.text();
    expect(html).toContain("每日最多3次");
    expect(html).toContain("认真完成5分");
    expect(html).toContain("必做2次");
    expect(html).toContain("前置任务：Read×2");
    expect(html).toContain("补做；可挽回2分；限时12小时");
    expect(html).not.toContain("Disabled task");
    expect(html).not.toContain("Disabled praise");
  });
});
