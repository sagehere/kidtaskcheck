...
describe("Task 35: Migration Smoke Test", () => {
  it("all 12 migration files apply sequentially on empty DB without errors", () => {
    const files = readdirSync(MIGRATIONS_DIR).filter((f) => f.endsWith(".sql")).sort();
    expect(files.length).toBe(13);
    const d1 = new D1Mock();
    for (const file of files) {
      const sql = readFileSync(join(MIGRATIONS_DIR, file), "utf8");
      expect(() => d1.exec(sql)).not.toThrow();
    }
    // Verify key tables exist
    const tables = d1.prepare("SELECT name FROM sqlite_master WHERE type='table' ORDER BY name").all().results as any[];
  ...
  it("migration 0012 does not drop data", () => {
    ...
    d1.exec("INSERT INTO users (id, username, password_hash, role, display_name) VALUES ('test-parent', 'tp', 'x', 'parent', 'TP')");
    d1.exec("INSERT INTO children (id, parent_id, username, password_hash, display_name, status) VALUES ('test-child', 'test-parent', 'tc', 'x', 'TC', 'active')");
    d1.exec("INSERT INTO ai_child_greetings (child_id, previous_week_key, config_hash, greeting) VALUES ('test-child', '2026-W22', 'abc123', 'Hello!')");
    const before = d1.prepare("SELECT COUNT(*) as count FROM ai_child_greetings").all().results as any[];
    ...
    d1.exec(sql12);
    const after = d1.prepare("SELECT COUNT(*) as count FROM ai_child_greetings").all().results as any[];
