...
export function buildAiPrompt(child, report, config, assignments) {
    ...
    if ((assignments?.tasks || []).length) {
        parts.push("任务配置：");
        for (const t of assignments.tasks.slice(0, 10))
...
export async function callAiService(env, prompt) {
    ...
    const apiKey = env.AI_API_KEY || (await env.DB.prepare("SELECT value FROM system_settings WHERE key='ai_api_key'").first())?.value || "";
...
export async function generateAiGreeting(env, child, offset, forceRefresh = false) {
    if (!child.ai_enabled)
        return "";
    const baseUrl = (await env.DB.prepare("SELECT value FROM system_settings WHERE key='ai_base_url'").first())?.value || "";
    const apiKey = env.AI_API_KEY || (await env.DB.prepare("SELECT value FROM system_settings WHERE key='ai_api_key'").first())?.value || "";
...
export async function batchRefreshGreetings(env, offset) {
    ...
    while (failed.length > 0 && attempt < AI_REFRESH_MAX_RETRIES) {
        ...
        for (const child of failed) {
            try {
                const greeting = await generateAiGreeting(env, child, offset, true);
                if (greeting)
                    successCount++;
                else
                    stillFailed.push(child);
...
export function isPrivateUrl(urlString) {
    try {
        ...
        if (h.startsWith("10.") || h.startsWith("172.") || h.startsWith("192.168.")) return true;
...
export const AI_MAX_REDIRECTS = 0;
